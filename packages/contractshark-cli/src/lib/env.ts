...
export function getConfig(argv?: any) {
  ...
  return {
    ...
    compiler: {
      build: getFirstValue(
        custom["build"],
        defaults["compiler"]["build"],
        "./build"
      ),
      match: getFirstValue(custom["match"], defaults["compiler"]["match"], []),
      severities: getFirstValue(
        custom["severities"],
        defaults["compiler"]["severities"],
        ["error", "warning"]
      ),
      evmVersion: getFirstValue(
        custom["evmVersion"],
        defaults["compiler"]["evmVersion"],
        "istanbul"
      ),
      clean: getFirstValue(
        custom["clean"],
        defaults["compiler"]["clean"],
        true
      ),
    },
    flattener: {
      build: getFirstValue(
        custom["build"],
        defaults["flattener"]["build"],
        "./build"
      ),
      match: getFirstValue(custom["match"], defaults["flattener"]["match"], []),
      severities: getFirstValue(
        custom["severities"],
        defaults["flattener"]["severities"],
        ["error", "warning"]
      ),
    },
    settings: {
      optimization: getFirstValue(
        custom["optimization"],
        defaults["compiler"]["optimization"],
        200
      ),
    },
    sandbox: {
    ...
    test: {
      server:
        custom["server"] !== false && defaults["test"]["server"] !== false,
      port: getFirstValue(custom["port"], defaults["test"]["port"], 8545),
      match: getFirstValue(custom["match"], defaults["test"]["match"], []),
      blockTime: getFirstValue(
        custom["blockTime"],
        defaults["test"]["blockTime"],
        null
      ),
      coverage:
        custom["coverage"] === true || defaults["test"]["coverage"] === true,
    },
