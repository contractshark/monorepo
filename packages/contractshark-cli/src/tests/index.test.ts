...
test("starts sandbox server", async (t) => {
  const command = `./bin/contractshark sandbox --port 8546 --ttl 1000`;
...
test("initializes current folder", async (t) => {
  ...
  const command = `mkdir -p ${cwd}; cd ${cwd}; ../../../bin/contractshark init --name foo --description bar; echo code: $?`;
  const { stdout, stderr } = await exec(command);
...
test("compiles smart contracts", async (t) => {
  ...
  const command = `./bin/contractshark compile --evmVersion byzantium --severities error warning --match ./src/tests/assets/*.sol --build ${dist}; echo code: $?`;
  const { stdout, stderr } = await exec(command);
...
test("flattens smart contracts", async (t) => {
  ...
  const command = `./bin/contractshark flatten --severities error warning --match ./src/tests/assets/*.sol --build ${dist}; echo code: $?`;
  const { stdout, stderr } = await exec(command);
  t.true(stdout.indexOf("Contracts") !== -1);
  t.true(stdout.indexOf("code: 0") !== -1);
  t.true(stderr === "");
...
test.serial("runs valid tests", async (t) => {
  const command = `./bin/contractshark test --match ./src/tests/assets/valid.hay.ts --require ts-node/register; echo code: $?`;
  const { stdout, stderr } = await exec(command);
  t.true(stdout.indexOf("code: 0") !== -1);
  t.true(stderr === "");
});
...
test.serial("runs invalid tests", async (t) => {
  const command = `./bin/contractshark test --match ./src/tests/assets/invalid.hay.ts --require ts-node/register; echo code: $?`;
  const { stdout, stderr } = await exec(command);
