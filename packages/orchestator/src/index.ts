...
const order = fs.readFileSync(
  path.join(__dirname, "/../assets/order.bpmn"),
  "utf-8"
);
const sample = fs.readFileSync(
  path.join(__dirname, "/../assets/sample.bpmn"),
  "utf-8"
);
...
app.get("/gateways", (req, res) => {
  parseModdle(sample).then((definitions) => {
    const choreography = <Choreography>(
      definitions.rootElements.find(is("bpmn:Choreography"))
    );
    if (!choreography) {
      return res.status(501).send("could not find a choreography instance");
    }
    const gateways = <Gateway[]>(
      choreography.flowElements.filter(is("bpmn:Gateway"))
    );
...
app.get("/neighbors/:id", (req, res) => {
  parseModdle(order).then((definitions) => {
    ...
    const successors = <FlowNode[]>(
      element.outgoing.map((flow) => flow.targetRef)
    );
    res.status(201).send({
