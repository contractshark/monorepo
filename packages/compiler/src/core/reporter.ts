...
export class DefaultReporter {
  ...
  protected onErrors(compiler: Compiler) {
    this.severities.forEach((severity) => {
      const failures = compiler.output.errors.filter(
  ...
  protected onContracts(compiler: Compiler) {
    ...
    if (contracts.length) {
      this.printer.end(this.printer.indent(1, ""), "Contracts");
  ...
  protected onOverview(compiler: Compiler) {
    ...
    if (this.severities.indexOf("error") !== -1) {
      const errors = (compiler.output.errors || []).filter(
        (e) => e.severity === "error"
      ).length;
    ...
    if (this.severities.indexOf("warning") !== -1) {
      const warnings = (compiler.output.errors || []).filter(
