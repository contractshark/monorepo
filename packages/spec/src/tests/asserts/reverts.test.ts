...
test("succeeds for values that revert with revert message check", async (t) => {
  t.true(
    reverts(() => {
      throw new Error("revert test message");
    }, "test message")
  );
  t.true(
    await reverts(() => Promise.reject("revert test message"), "test message")
  );
  ...
  t.false(
    await reverts(() => Promise.reject("revert test message"), "testing")
  );
});
