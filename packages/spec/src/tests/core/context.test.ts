...
test("method `deploy` deploys contract from JSON file", async (t) => {
  ...
  t.is(
    contact.instance.options.from,
    await stage.web3.eth.getAccounts().then((a) => a[0])
  );
...
test("method `sign` creates a signature and validates it", async (t) => {
  ...
  const address = await recover.instance.methods
    .recover(data, signatureData.r, signatureData.s, signatureData.v)
    .call();
